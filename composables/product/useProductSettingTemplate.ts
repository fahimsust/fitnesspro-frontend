export const useProductSettingTemplate = () => {
  ...
  async function list() {
    const settingList: any = await useAsyncData("productSettingsTemplate", () =>
  ...
  async function deleteSettingTemplate() {
    try {
      await $larafetch(`/product-settings-template/${archiveId}`, { method: "delete" });
      swalTopEnd(
  ...
  async function updateSettingTemplate(formData: object, id: number) {
    ...
    const result: any = await submitRequest(
      $larafetch(`/product-settings-template/${id}`, {
        method: "put",
        body: formData,
      }),
      (_: any) => {
