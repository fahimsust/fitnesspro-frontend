...
import { type Menu } from "../../store/enigma/menus/side-menu";
  ...
  subMenu.forEach((item) => {
    if (
      ((route.forceActiveMenu !== undefined &&
        ...
        (route.forceActiveMenu === undefined &&
          (item.pageName === getfullPath(route) || item.pageName === route.name || getReplacedPath(route).indexOf(item.pageName!) != -1  ))) &&
      !item.ignore
    ) {
...
const getReplacedPath = (route: Route)=>{
  let path = "/"
  if(route.name)
  {
    path += route.name!.toString().replaceAll("-","/")
  }
  return path;
}
