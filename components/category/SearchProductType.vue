...
const { searchProductType } = useCategoryProductType();
const idParem = parseInt(route.params.id.toString());
const searchByKeyword = async (value: string) => {
  keyword.value = value;
  currentPage.value = 1;
  await searchProductType(idParem, keyword.value);
};
const list = async () => {
  await searchProductType(idParem, keyword.value);
};
...
onMounted(async () => {
  await nextTick();
  const result = await searchProductType(
    idParem,
    keyword.value
  );
...
<template>
  <Dialog size="xl" :open="modalShow" @close="() => {
    ...
    <Dialog.Panel class="p-3 text-center">
      ...
      <Dialog.Title>
        <h2 class="mr-auto text-base font-medium">Search Product Types</h2>
      </Dialog.Title>
          ...
          <Table class="border-spacing-y-[10px] border-separate -mt-2">
            ...
            <Table.Tbody>
              ...
              <Table.Tr v-for="productType in productTypes" @click="() => {
                if (!awaiting) addAndSearch(productType.id);
              }
