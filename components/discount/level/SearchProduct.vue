...
const { searchDiscountLevelProduct } = useDiscountLevelProduct();
const idParem = parseInt(route.params.id.toString());
const searchByKeyword = async (value: string) => {
  keyword.value = value;
  currentPage.value = 1;
  await searchDiscountLevelProduct(idParem, keyword.value);
};
const list = async () => {
  await searchDiscountLevelProduct(idParem, keyword.value);
};
...
onMounted(async () => {
  await nextTick();
  const result = await searchDiscountLevelProduct(
    idParem,
    keyword.value
  );
          ...
          <Table class="border-spacing-y-[10px] border-separate -mt-2">
            ...
            <Table.Tbody>
              ...
              <Table.Tr v-for="product in products" @click="() => {
                if (!awaiting) addAndSearch(product.id);
              }
