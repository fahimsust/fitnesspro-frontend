...
const { searchConditionMemberships } = useConditionMembership();
const searchByKeyword = async (value: string) => {
  keyword.value = value;
  currentPage.value = 1;
  await searchConditionMemberships(condition_id.value, keyword.value);
};
const list = async () => {
  await searchConditionMemberships(condition_id.value, keyword.value);
};
...
onMounted(async () => {
  await nextTick();
  const result = await searchConditionMemberships(
    condition_id.value,
    keyword.value
  );
...
<template>
  <Dialog size="xl" :open="modalShow" @close="() => {
    ...
    <Dialog.Panel class="p-3">
      ...
      <Dialog.Title>
        <h2 class="mr-auto text-base font-medium">Search Membership Level</h2>
      </Dialog.Title>
        ...
        <div class="overflow-y-auto col-span-12">
          <Table class="border-spacing-y-[10px] border-separate -mt-2">
            ...
            <Table.Tbody>
              ...
              <Table.Tr v-if="memberships.length == 0 && dataLoaded" class="intro-x">
                <Table.Td colspan="2" class="py-0 px-0">
                  <AlertGray />
                </Table.Td>
              </Table.Tr>
              <Table.Tr v-for="membership in memberships" @click="() => {
                if (!wait) addAndSearch(membership.id);
              }
                " :key="membership.id" class="intro-x hover:bg-primary hover:text-white"
