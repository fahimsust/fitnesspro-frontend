...
const { searchConditionOutOfStockStatus } = useConditionOutOfStockStatus();
const searchByKeyword = async (value: string) => {
  keyword.value = value;
  await searchConditionOutOfStockStatus(condition_id.value, keyword.value);
};
...
let availabilities: any = ref([])
const dataLoaded = ref(false)
onMounted(async () => {
  await nextTick();
  availabilities = await searchConditionOutOfStockStatus(
    condition_id.value,
    keyword.value
  );
...
<template>
  <Dialog size="xl" :open="modalShow" @close="() => {
    ...
    <Dialog.Panel class="p-3">
      ...
      <Dialog.Title>
        <h2 class="mr-auto text-base font-medium">Search Product out of stock status</h2>
      </Dialog.Title>
        ...
        <div class="overflow-y-auto col-span-12">
          <Table class="border-spacing-y-[10px] border-separate -mt-2">
            ...
            <Table.Tbody>
              ...
              <Table.Tr v-if="availabilities.length == 0 && dataLoaded" class="intro-x">
                <Table.Td colspan="2" class="py-0 px-0">
                  <AlertGray />
                </Table.Td>
              </Table.Tr>
              <Table.Tr v-for="availability in availabilities" @click="() => {
                if (!wait) addAndSearch(availability.id);
              }
                " :key="availability.id" class="intro-x hover:bg-primary hover:text-white"
