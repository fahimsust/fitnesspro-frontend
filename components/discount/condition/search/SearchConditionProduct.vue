...
const { searchConditionProducts } = useConditionProduct();
const searchByKeyword = async (value: string) => {
  keyword.value = value;
  currentPage.value = 1;
  await searchConditionProducts(condition_id.value, keyword.value);
};
const list = async () => {
  await searchConditionProducts(condition_id.value, keyword.value);
};
...
onMounted(async () => {
  await nextTick();
  const result = await searchConditionProducts(
    condition_id.value,
    keyword.value
  );
    ...
    " class="absolute top-3 right-4" href="#">
      ...
      <Dialog.Title>
        <h2 class="mr-auto text-base font-medium">
          Search Product For Discount Condition
        </h2>
      </Dialog.Title>
