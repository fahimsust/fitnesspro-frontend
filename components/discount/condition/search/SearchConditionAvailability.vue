...
const { searchConditionAvailability } = useConditionProductAvailability();
const searchByKeyword = async (value: string) => {
  keyword.value = value;
  await searchConditionAvailability(condition_id.value, keyword.value);
};
...
onMounted(async () => {
  await nextTick();
  availabilities = await searchConditionAvailability(
    condition_id.value,
    keyword.value
  );
...
<template>
  <Dialog size="xl" :open="modalShow" @close="() => {
    ...
    <Dialog.Panel class="p-3">
      ...
      <Dialog.Title>
        <h2 class="mr-auto text-base font-medium">Search Product availability</h2>
      </Dialog.Title>
