...
const { searchConditionSite } = useConditionSite();
const searchByKeyword = async (value: string) => {
  keyword.value = value;
  await searchConditionSite(condition_id.value, keyword.value);
};
...
onMounted(async () => {
  await nextTick();
  sites = await searchConditionSite(
    condition_id.value,
    keyword.value
  );
        ...
        <div class="overflow-y-auto col-span-12">
          <Table class="border-spacing-y-[10px] border-separate -mt-2">
            ...
            <Table.Tbody>
              ...
              <Table.Tr v-for="site in sites" @click="() => {
                if (!wait) addAndSearch(site.id);
              }
                " :key="site.id" class="intro-x hover:bg-primary hover:text-white"
