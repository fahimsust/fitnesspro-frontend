...
const { searchConditionProductType } = useConditionProductType();
const searchByKeyword = async (value: string) => {
  keyword.value = value;
  currentPage.value = 1;
  await searchConditionProductType(condition_id.value, keyword.value);
};
const list = async () => {
  await searchConditionProductType(condition_id.value, keyword.value);
};
...
onMounted(async () => {
  await nextTick();
  const result = await searchConditionProductType(
    condition_id.value,
    keyword.value
  );
    ...
    " class="absolute top-3 right-4" href="#">
      ...
      <Dialog.Title>
        <h2 class="mr-auto text-base font-medium">
          Search Product Type For Discount Condition
        </h2>
      </Dialog.Title>
        ...
        <div class="overflow-y-auto col-span-12">
          <Table class="border-spacing-y-[10px] border-separate -mt-2">
            ...
            <Table.Tbody>
              ...
              <Table.Tr v-if="productTypes.length == 0" class="intro-x">
