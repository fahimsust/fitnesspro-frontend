...
const { searchAdvantageProducts } = useAdvantageProduct();
const searchByKeyword = async (value: string) => {
  keyword.value = value;
  currentPage.value = 1;
  await searchAdvantageProducts(advantage_id.value, keyword.value);
};
const list = async () => {
  await searchAdvantageProducts(advantage_id.value, keyword.value);
};
...
watch(() => advantage_id.value, async (newVal) => {
  if (newVal) {
    await searchAdvantageProducts(advantage_id.value, keyword.value);
  }
});
...
onMounted(async () => {
  await nextTick();
  const result = await searchAdvantageProducts(
    advantage_id.value,
    keyword.value
  );
...
<template>
  <Dialog size="xl" :open="modalShow" @close="() => {
    ...
    <Dialog.Panel class="p-3">
      ...
      <Dialog.Title>
        <h2 class="mr-auto text-base font-medium">Search Product For Discount Advantage</h2>
      </Dialog.Title>
