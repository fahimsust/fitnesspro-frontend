...
const keyAmount = ref(1)
const { advantage } = toRefs(props);
const amountKey = ref(keyAmount.value.toString() + "-" + advantage.value.id.toString())
...
watch(() => advantageValue.value, async (newVal) => {
  if (newVal) {
    keyAmount.value += 1;
    amountKey.value = keyAmount.value.toString() + "-" + advantage.value.id.toString()
  }
});
</script>
...
<template>
  ...
  <div class="col-span-12 intro-y md:col-span-4">
    <InputField :change="() => updated = true" :blur="validateAndUpdate" :submitFunction="validateAndUpdate" name="amount"
      v-model="formEdit" :error="errors.amount" title="Amount Off" />
  </div>
  <div class="col-span-12 mt-3">
    <AdvantageProduct :applyto_qty_type="formEdit.applyto_qty_type" :key="amountKey" :advantage="advantageValue"
      :refresh="refresh" />
  </div>
</template>