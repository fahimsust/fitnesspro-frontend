...
import PaginationComponent from "./enigma/base-components/Pagination";
...
const { numberOfPages, modelValue,getList } = toRefs(props)

let currentPage = ref(modelValue.value)
const maxPages = 3;
...
async function createNewPath(number: Number): Promise<void> {
    ...
    createNewPathAndPush("page",number.toString(),false);
    scrollToTop();
    await getList.value();
...
<template>
    <div v-if="numberOfPages > 1" class="flex flex-wrap items-center col-span-12 intro-y sm:flex-row sm:flex-nowrap">
        <PaginationComponent :key="currentPage">
            <PaginationComponent.Link v-if="currentPage > 2" @click="setCurrentPage(1)">
                <Lucide icon="ChevronsLeft" class="w-4 h-5" />
            </PaginationComponent.Link>

            <PaginationComponent.Link v-if="currentPage != 1" @click="previous">
            ...
            <PaginationComponent.Link v-if="showNextLink" @click="next">
                <Lucide icon="ChevronRight" class="w-4 h-5" />
            </PaginationComponent.Link>

            <PaginationComponent.Link v-if="showLastPage" @click="setCurrentPage(lastPage)">
