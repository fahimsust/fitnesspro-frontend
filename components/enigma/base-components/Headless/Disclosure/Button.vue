...
if (group) {
  watch(group, () => {
    if (!document.getElementById("overflow-div-id"))
...
const computedClass = computed(() =>
  twMerge([
    ...
    disclosure?.value.open && "text-success font-bold dark:text-slate-300",
...
<template>
  <HeadlessDisclosureButton as="template" @click="() => {
    ...
    ">
    <component :is="as" :class="computedClass" v-bind="{ ...attrs, class: undefined }">
      <slot></slot>
    </component>
