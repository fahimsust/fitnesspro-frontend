...
import { useProductAccessoryField } from "~~/composables/product/useProductAccessoryField";
...
const { searchProductAccessoryField } = useProductAccessoryField();
const searchByKeyword = async (value: string) => {
  keyword.value = value;
  currentPage.value = 1;
  await searchProductAccessoryField(idParem, keyword.value);
};
const list = async () => {
  await searchProductAccessoryField(idParem, keyword.value);
};
...
let accessoryFields: any = ref([])
let lastPage: any = ref(1)
let currentPage: any = ref(1)
const dataLoaded = ref(false)
onMounted(async () => {
  await nextTick();
  const result = await searchProductAccessoryField(
    idParem,
    keyword.value
  );
  accessoryFields = result.accessoryFields
...
<template>
  <Dialog size="xl" :open="modalShow" @close="() => {
    ...
    <Dialog.Panel class="p-3">
      ...
      <Dialog.Title>
        <h2 class="mr-auto text-base font-medium">Search Accessory Field</h2>
      </Dialog.Title>
          ...
          <Table class="border-spacing-y-[10px] border-separate -mt-2">
            <Table.Thead variant="dark">
              <Table.Tr>
                ...
                <Table.Th class="whitespace-nowrap">FIELD TYPE</Table.Th>
            ...
            <Table.Tbody>
              ...
              <Table.Tr v-if="accessoryFields.length == 0 && dataLoaded" class="intro-x">
                <Table.Td colspan="4" class="py-0 px-0">
                  <AlertGray />
                </Table.Td>
              </Table.Tr>
              <Table.Tr v-for="accessoryField in accessoryFields" @click="() => {
                if (!wait) addAndSearch(accessoryField.id);
              }
                " :key="accessoryField.id" class="intro-x hover:bg-primary hover:text-white"
                ...
                <Table.Td>
                  {{
                    accessoryField.field_type == 1
                      ? "select menu"
                      : accessoryField.field_type == 2
                        ? "radio options"
                        : "checkboxes"
                  }}
                </Table.Td>
                <Table.Td>
                  {{ accessoryField.required == 1 ? "Yes" : "No" }}
                </Table.Td>
              </Table.Tr>
            </Table.Tbody>
          </Table>
