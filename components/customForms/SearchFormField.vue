...
const { setModel, modalShow, add, section_id } = toRefs(props);
...
const addAndSearch = async (name: string) => {
  wait.value = true;
  await add.value(name, section_id.value);
  wait.value = false;
};

const dataLoaded = ref(false)
let customFields:any = ref([]);
let lastPage:any = ref(1);
let currentPage:any = ref(1);

const fetchData = async ()=>{
  const result = await list(keyword.value);
...
<template>
  <Dialog size="xl" :open="modalShow" @close="() => {
    ...
    <Dialog.Panel class="p-3">
      ...
      <Dialog.Title>
        <h2 class="mr-auto text-base font-medium">Search Custom Field</h2>
      </Dialog.Title>
        ...
        <div class="overflow-y-auto col-span-12">
          <Table class="border-spacing-y-[10px] border-separate -mt-2">
            ...
            <Table.Tbody>
              ...
              <Table.Tr v-if="customFields.length == 0 && dataLoaded" class="intro-x">
                <Table.Td colspan="3" class="py-0 px-0">
                  <AlertGray />
                </Table.Td>
              </Table.Tr>
              <Table.Tr v-for="customField in customFields" @click="() => {
                if (!wait) addAndSearch(customField.name);
              }
                " :key="customField.id" class="intro-x hover:bg-primary hover:text-white"
