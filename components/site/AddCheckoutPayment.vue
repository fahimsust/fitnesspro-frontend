...
import { SelectField, SubmitButton, CancelButton, InputField } from "~/components/form";
...
const { addCheckoutSitePaymentMethod, errors, loading } = useSitePaymentMethodCheckout();
...
const paymentMethodValidationAndAdd = async () => {
  await addCheckoutSitePaymentMethod(form, idParem);
      ...
      <Dialog.Description>
        <div v-if="dataLoaded">
          <SelectField :options="paymentMethods" name="payment_method_id" type="text" v-model="form"
            :runFunction="getPaymentAccountsFromMethod" :error="errors.payment_method_id" title="Payment Method" />
        </div>
        ...
        <div class="mt-3">
          <InputField name="fee" type="text" v-model="form" :error="errors.fee" title="Fee" />
        </div>
      </Dialog.Description>
      <Dialog.Footer>
