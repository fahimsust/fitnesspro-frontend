        ...
        <div class="overflow-y-auto col-span-12">
          <Table class="border-spacing-y-[10px] border-separate -mt-2">
            ...
            <Table.Tbody>
              ...
              <Table.Tr v-if="currencies.length == 0 && dataLoaded" class="intro-x">
                <Table.Td colspan="3" class="py-0 px-0">
                  <AlertGray />
                </Table.Td>
              </Table.Tr>
              <Table.Tr v-for="currency in currencies" @click="() => {
                if (!wait) addAndSearch(currency.id);
              }
                " :key="currency.id" class="intro-x hover:bg-primary hover:text-white"
