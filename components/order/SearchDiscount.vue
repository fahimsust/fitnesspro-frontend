...
const { setModel, modalShow, add, order_id } = toRefs(props);
const keyword = ref("");
const wait = ref(false);
const { discountList } = useOrderDiscounts();
const searchByKeyword = async (value: string) => {
  keyword.value = value;
  currentPage.value = 1;
  await discountList(keyword.value, order_id.value);
};
const list = async () => {
  await discountList(keyword.value, order_id.value);
};
...
<template>
  <Dialog size="xl" :open="modalShow" @close="() => {
    ...
    <Dialog.Panel class="p-3">
      ...
      <Dialog.Title>
        <h2 class="mr-auto text-base font-medium">Search Discounts</h2>
      </Dialog.Title>
      <Dialog.Description class="grid grid-cols-12 gap-4 gap-y-3 max-h-[72vh] overflow-auto">
        ...
        <div class="overflow-y-auto col-span-12">
          <Table class="border-spacing-y-[10px] border-separate -mt-2">
            ...
            <Table.Tbody>
              ...
              <Table.Tr v-for="discount in discounts" @click="() => {
                if (!wait) addAndSearch(discount.id);
              }
                " :key="discount.id" class="intro-x hover:bg-primary hover:text-white"
