...
const { searchOrderAccount } = useOrderAccounts();
const searchByKeyword = async (value: string) => {
  keyword.value = value;
  currentPage.value = 1;
  await searchOrderAccount(keyword.value);
};
const list = async () => {
  await searchOrderAccount(keyword.value);
};
...
const fetchData = async () => {
  const result = await searchOrderAccount(
    keyword.value
  );
...
<template>
  <Dialog size="xl" :open="modalShow" @close="() => {
    ...
    <Dialog.Panel class="p-3">
      ...
      <Dialog.Title>
        <h2 class="mr-auto text-base font-medium">Search Customer</h2>
      </Dialog.Title>
        ...
        <div class="overflow-y-auto col-span-12">
          <Table class="border-spacing-y-[10px] border-separate -mt-2">
            ...
            <Table.Tbody>
              ...
              <Table.Tr v-for="account in accounts" @click="() => {
                if (!wait) addAndSearch(account.id);
              }
                " :key="account.id" class="intro-x hover:bg-primary hover:text-white"
