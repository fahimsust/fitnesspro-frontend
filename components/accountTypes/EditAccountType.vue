...
const { updateAccountType, errors, loading, messageTemplates, loyaltyPrograms } = useAccountType();
const { setModal, modalShow, modelValue: accountType, refresh } = toRefs(props);
...
const fetchData = async () => {
  ...
  form = reactive({
    name: accountType.value.name,
    default_account_status: accountType.value.default_account_status,
    custom_form_id: accountType.value.custom_form_id,
    email_template_id_creation_admin: accountType.value.email_template_id_creation_admin,
    email_template_id_creation_user: accountType.value.email_template_id_creation_user,
    email_template_id_activate_user: accountType.value.email_template_id_activate_user,
    discount_level_id: accountType.value.discount_level_id,
    verify_user_email: accountType.value.verify_user_email,
    filter_products: accountType.value.filter_products ? accountType.value.filter_products : 0,
    filter_categories: accountType.value.filter_categories ? accountType.value.filter_categories : 0,
    loyaltypoints_id: accountType.value.loyaltypoints_id,
    use_specialties: accountType.value.use_specialties,
    membership_level_id: accountType.value.membership_level_id,
    email_template_id_verify_email: accountType.value.email_template_id_verify_email,
    affiliate_level_id: accountType.value.affiliate_level_id

  });
  dataLoaded.value = true
}

const typeValidationAndEdit = async () => {
  await updateAccountType(form, accountType.value.id);
  await refresh.value();
...
<template>
  <Dialog backdrop="static" :open="modalShow" size="xl" @close="() => {
    ...
    <Dialog.Panel>
      ...
      <Dialog.Title>
        <h2 class="mr-auto text-base font-medium">Edit Account Type</h2>
      </Dialog.Title>
      ...
      <Dialog.Description class="max-h-[75vh] overflow-auto">
        ...
        <div class="grid grid-cols-12 gap-4 gap-y-3" v-if="dataLoaded">
          ...
          <div class="mt-3 col-span-6 xs:col-span-12">
            <SelectField :options="templates" name="email_template_id_creation_admin" v-model="form"
              defaultOption="None" :error="errors.email_template_id_creation_admin"
              title="Email to Admin On Creation" />
          </div>
          <div class="mt-3 col-span-6 xs:col-span-12">
      ...
      <Dialog.Footer>
        <SubmitButton :createFunction="typeValidationAndEdit" :loading="loading" />
      </Dialog.Footer>
    </Dialog.Panel>
  </Dialog>
</template>
