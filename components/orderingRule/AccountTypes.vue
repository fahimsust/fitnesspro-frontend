...
<template>
  <Dialog :key="condition_id" size="xl" :open="modalShow" @close="() => {
    setModel(false);
  }
    ">
    <Dialog.Panel class="p-3">
      ...
      <Dialog.Title>
        <h2 class="mr-auto text-base font-medium">Search Account Types</h2>
      </Dialog.Title>
          ...
          <Table class="border-spacing-y-[10px] border-separate -mt-2">
            ...
            <Table.Tbody>
              ...
              <Table.Tr v-for="accountType in accountTypes" @click="() => {
                if (!processing) addAndReload(accountType.id);
              }
                " :key="accountType.id" :class="{ 'cursor-pointer': !processing, 'cursor-wait': processing }"
